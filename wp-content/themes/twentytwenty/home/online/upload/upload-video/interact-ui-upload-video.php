...
for (let pagingIndex = 0 ; pagingIndex < totalPage ; pagingIndex++ ){
	...
	function createTableUploadVideo(tboby,input,number){
		...
		trContent.innerHTML = "<td>"+(index)+"</td><td>"+item.title+"</td><td class=\"tdShortDesscription\"><div class=\"divShortDesscription\">"+(item.shortDescription != null ? item.shortDescription : "Thiếu")+"</div></td><td class=\"tdVideoStatus\">"+status+"</td><td class='manage-list-teacher-table-detail-tr-modified'>" + editVideo + "<div class='manage-list-teacher-table-detail-div-delete manage-list-teacher-table-detail-div-delete-video' onclick=\"deleteUploadVideo('"+item.id+"')\">Xóa</div></td>";
