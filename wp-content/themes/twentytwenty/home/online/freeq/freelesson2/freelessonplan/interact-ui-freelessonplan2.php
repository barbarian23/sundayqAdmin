...
for (let pagingIndex = 0 ; pagingIndex < totalPage ; pagingIndex++ ){
	...
	function createTableFreeLessonPlan2(tboby,input,number){
		...
		trContent.innerHTML = "<td>"+(index)+"</td><td>"+item.title+"</td><td class=\"tdShortDesscription\">"+(item.shortDescription != null ? item.shortDescription : "Thiếu")+"</td><td class='manage-list-teacher-table-detail-tr-modified'><div class='manage-list-teacher-table-detail-div-edit'><a href=\"" + tempAHref + "&month="+item.month+"\">Chỉnh sửa</a></div><div class='manage-list-teacher-table-detail-div-delete' onclick=\"deleteFreeLessonPlan2("+(index-1)+")\">Xóa</div></td>";
